...
export default function Login({styles, submit, isLoading, setIsLogin}){
    ...
    const animationRepeatPass = useAnimationInput();

    return(
        <div className={styles.container}>
            <div style={{display:"flex", justifyContent:"flex-start", padding:"1rem"}}>
                <h2 className={styles.titleText}>Ingresa tus datos para crear una cuenta</h2>
            </div>
            <form onSubmit={submit}>
                ...
                <div className={styles.inputField}>
                    <label 
                        htmlFor="repeatPass"
                        className={`${animationRepeatPass.inputFocus ? styles.labelBlur : styles.labelFocus}`}   
                    >
                        Repetir contraseña
                    </label>
                    <input 
                        id="repeatPass" 
                        type="text" 
                        className={`${styles.textInput} ${animationRepeatPass.inputFocus && styles.textInputBlur}`}
                        onFocus={() => animationRepeatPass.focusAnimation()}
                        onBlur={(e) => animationRepeatPass.blurAnimation(e)}
                    />
                </div>
            
                <div className={styles.inputField} style={{display:"flex", justifyContent:"flex-end"}}>
                    <button type="button" onClick={() => alert("Funcion no habilitada")} style={{ padding: "8px" }} className="buttonsHot">
                        {!isLoading ? (
                            "Entrar"
                        ):(
            ...
            <div className={styles.optionsAuthContainer} style={{justifyContent:"flex-end"}}>
                <p onClick={() => setIsLogin(true)}>iniciar sesion</p>
            </div>
        </div>
    )
}